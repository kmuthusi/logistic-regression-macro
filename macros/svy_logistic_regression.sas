...
data xx_dataset;
 ...
 %* show or suppress missing values;
	...
	%if %length(&condition) ne 0 & %length(&weight) ne 0 %then %do;
		&condition and &weight > 0; 
	%end; 

	%else %if %length(&condition) eq 0 & %length(&weight) ne 0 %then %do; 
		if &weight > 0;
	%end;

	%else %if %length(&condition) ne 0 % %length(&weight) eq 0 %then %do; 
		&condition; 
	%end;
...
data _nfreq;
	...
	if trim(left(f_&outcome)) eq "&outevent" then output;
...
data _tfreq;
	...
	_tfreq=Frequency;
	_tpercent=Percent;
	if trim(left(f_&outcome)) eq "Total" then output;
...
data _nfreqn;
set _nfreqn;
	if &domain eq &domvalue and trim(left(f_&outcome))="&outevent" then output;
run;
